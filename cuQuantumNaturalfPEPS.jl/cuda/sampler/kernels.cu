...
{
    __shared__ qnpeps::CuArray<f32, k_tree_reduce_threads> shared_max;
    ...
    for (auto index = threadIdx.x; index < args.n; index += blockDim.x)
    {
        const auto component_abs_sum =
            cuda::std::abs(values[index].x) + cuda::std::abs(values[index].y);
    ...
    if (threadIdx.x == 0)
    {
        if (args.lognorm_acc) args.lognorm_acc[lane] += cuda::std::log(static_cast<f64>(scale));
...
__global__ auto cu_draw(CuDrawArgs args) -> void
{
    ...
    const auto is_uniform = total_weight <= 0.0f or not cuda::std::isfinite(total_weight);
    if (is_uniform) total_weight = static_cast<f32>(dim_phys);

    DeviceRandomState rng_state{};
    initialize_random(
        rng_state, seed, static_cast<u64>(args.lane_base + lane), static_cast<u64>(site_counter)
    );
    const f32 uniform_draw{random_uniform(rng_state)};

    f32 cumulative{};
    int chosen{dim_phys - 1};
    for (auto phys = 0; phys < dim_phys; ++phys)
    {
        const f32 weight{is_uniform ? 1.0f : magnitude(rho_lane[diag_index(phys, dim_phys)])};
    ...
    const f32 chosen_weight{is_uniform ? 1.0f : magnitude(rho_lane[diag_index(chosen, dim_phys)])};
    const auto chosen_prob = static_cast<f64>(chosen_weight / total_weight);
    logpc[lane] += cuda::std::log(chosen_prob);
...
__global__ auto cu_chol_shift_roundoff(CuCholShiftRoundoffArgs args) -> void
{
    const auto lane = static_cast<int>(global_lane());
    if (lane >= args.dim_batch) return;

    auto* gram_lane = args.gram + lane * args.stride;
    f32 trace{};
    for (auto diag = 0; diag < args.k; ++diag)
        trace += gram_lane[diag_index(diag, args.k)].x;
    constexpr f32 epsilon{1.1920928955078125e-7f};
    constexpr f32 unit_roundoff{0.5f * epsilon};
    const f32 scale{
        args.conservative_bound
            ? 11.0f
                  * static_cast<f32>(
                      static_cast<i64>(args.rows) * args.k + static_cast<i64>(args.k) * (args.k + 1)
                  )
                  * unit_roundoff
            : 10.0f * epsilon
    };
    const f32 shift{(trace > 0.0f ? scale * trace : 0.0f) + 1.0e-30f};
    for (auto diag = 0; diag < args.k; ++diag)
        gram_lane[diag_index(diag, args.k)].x += shift;
}
...
__global__ auto cu_fill_first_one(CuFillFirstOneArgs args) -> void
{
    const i64 total{static_cast<i64>(args.n) * args.dim_batch};
    for (i64 tid{global_lane()}; tid < total; tid += grid_stride())
    {
        const auto elem = static_cast<int>(tid % args.n);
        const i64 lane{tid / args.n};
        const i64 out_idx{lane * args.stride + elem};
        args.x[out_idx] = cuFloatComplex{elem == 0 ? 1.0f : 0.0f, 0.0f};
    }
}
