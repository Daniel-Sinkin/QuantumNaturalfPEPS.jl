...
class Shape
{
  ...
  private:
    [[nodiscard]] static auto is_shape(const std::vector<int>& extents) noexcept -> bool
    {
        ...
        {
            const auto invalid_extent = extent <= 0;
            const auto product_overflows =
                total > std::numeric_limits<usize>::max() / static_cast<usize>(extent);
            if (invalid_extent or product_overflows)
            {
                return false;
            }
            total *= static_cast<usize>(extent);
