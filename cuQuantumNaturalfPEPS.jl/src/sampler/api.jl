...
function _sample_count(n_samples::Integer)::Int
    n_samples >= 0 || throw(ArgumentError("n_samples must be nonnegative"))
    return Int(n_samples)
end

function _batch_base(batch_base::Integer)::UInt64
...
function _sample_all(
    ...
    chi_c::Integer,
    route,
    sampler_density_cutoff::Real,
    projected_density_cutoff::Real,
    batch_size::Integer,
    batch_base::Integer,
)::SampleResult
    sample_count = _sample_count(n_samples)
    batch_base = _batch_base(batch_base)
    ...
    config = _cfg_of(
        ...
        sampling_mode,
        chi_c,
        sampler_truncation_route=route,
        sampler_density_cutoff,
        projected_density_cutoff,
    )
...
function _sample_peps_one_shot(
    ...
    context = SamplerContext(peps; chi_s, chi_dl, seed, sampling_mode, chi_c, batch_size)
    try
        samples = sample_peps(context, n_samples; batch_base)
        host_dlenv =
            tensors === nothing ? materialize_dlenv(context) : materialize_dlenv(context, tensors)
        return (
            dlenv=host_dlenv,
...
function sample_peps!(
    ...
    sample_count = div(length(samples), Int(config.lx) * Int(config.ly))
    dim_batch = _sample_batch_size(sample_count, batch_size)
    seeded = _reseed(config, seed)
    num_sites = Int(seeded.lx) * Int(seeded.ly)
    length(samples) % num_sites == 0 ||
        throw(DimensionMismatch("sample buffer length is not divisible by the PEPS site count"))
    log_prob_config_ptr = log_prob_config !== nothing ? pointer(log_prob_config) : CuPtr{Float64}(0)
    log_gauge_ptr = log_gauge !== nothing ? pointer(log_gauge) : CuPtr{Float64}(0)
    GC.@preserve peps_data dlenv_data samples log_prob_config log_gauge begin
        _ffi_sample(;
            config=seeded,
