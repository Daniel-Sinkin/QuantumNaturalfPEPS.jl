...
function _validate_context_peps(context::SamplerContext, device_peps::CuPeps)::Nothing
    ...
    dimensions_match || throw(DimensionMismatch("$device_peps does not match $context"))
    return nothing
end
...
function _context_dlenv_arrays(context::SamplerContext)::Vector{Vector{Array{ComplexF32,4}}}
    _validate_sampler_context(context)
...
function materialize_dlenv(context::SamplerContext, tensors::AbstractMatrix)::Vector{MPS}
