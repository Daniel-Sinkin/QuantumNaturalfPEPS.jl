...
function _zipup_site_dims(
    ...
    for (site_index, site) in enumerate(sites)
        ...
        eltype(site) == ComplexF32 ||
            throw(ArgumentError("$label site $site_index must have element type ComplexF32"))
        for extent in size(site)
            1 <= extent <= typemax(Int32) ||
                throw(DimensionMismatch("$label site $site_index has invalid extent $extent"))
...
function _zipup_validate_topology(
    ...
    length(mpo) == length(mps) ||
        throw(DimensionMismatch("MPO and MPS site counts differ: $(length(mpo)) != $(length(mps))"))
    ...
    for site in 1:length(mpo)
        ...
        mpo_left == expected_mpo_left ||
            throw(DimensionMismatch("MPO bond before site $site is not open/adjacent"))
        mps_left == expected_mps_left ||
            throw(DimensionMismatch("MPS bond before site $site is not open/adjacent"))
        mpo_physical_in == mps_physical || throw(
            DimensionMismatch("MPO physical input and MPS physical dimension differ at site $site"),
        )
        if site == length(mpo)
...
function _zipup_site_indices(
    ...
    isempty(physical) && throw(DimensionMismatch("$label site $site has no physical index"))
    return physical
end
...
function _zipup_itensor_host_arrays(mpo::MPO, mps::MPS)::NamedTuple
    ...
    for site in eachindex(layout)
        ...
        mpo_indices = Tuple(
            index for
            index in (axes.mpo_left, axes.physical_input, axes.physical_output, axes.mpo_right) if
            index !== nothing
        )
        mps_indices = Tuple(
            index for
...
function _zipup_arrays_to_mps(sites::AbstractVector, physical_outputs::AbstractVector{<:Index})::MPS
    ...
    for site in 1:(length(sites)-1)
        size(sites[site], 3) == size(sites[site+1], 1) ||
            throw(DimensionMismatch("output MPS bond after site $site is not adjacent"))
        links[site] = Index(size(sites[site], 3); tags="Link,l=$site")
    end
    tensors = Vector{ITensor}(undef, length(sites))
    for site in eachindex(sites)
        ndims(sites[site]) == 3 ||
            throw(DimensionMismatch("output MPS site $site must have rank three"))
        ...
        left == expected_left ||
            throw(DimensionMismatch("output MPS left dimension is invalid at site $site"))
        right == expected_right ||
            throw(DimensionMismatch("output MPS right dimension is invalid at site $site"))
        physical == dim(physical_outputs[site]) ||
            throw(DimensionMismatch("output MPS physical dimension is invalid at site $site"))
        values = ComplexF32.(Array(sites[site]))
        if length(sites) == 1
            tensors[site] = itensor(reshape(values, physical), physical_outputs[site])
        elseif site == 1
            tensors[site] =
                itensor(reshape(values, physical, right), physical_outputs[site], links[site])
        elseif site == length(sites)
            tensors[site] =
                itensor(reshape(values, left, physical), dag(links[site-1]), physical_outputs[site])
        else
            tensors[site] = itensor(values, dag(links[site-1]), physical_outputs[site], links[site])
        end
    end
    return MPS(tensors)
end

function zipup_mpo_mps(mpo::AbstractVector, mps::AbstractVector; maxdim::Integer)::NamedTuple
    1 <= maxdim <= typemax(Int32) ||
        throw(ArgumentError("maxdim must be in 1:$(typemax(Int32)) (got $maxdim)"))
    length(mpo) == length(mps) ||
        throw(DimensionMismatch("MPO and MPS site counts differ: $(length(mpo)) != $(length(mps))"))
    isempty(mpo) && throw(ArgumentError("MPO and MPS must contain at least one site"))
...
function zipup_mpo_mps(mpo::MPO, mps::MPS; maxdim::Integer)::NamedTuple
    host = _zipup_itensor_host_arrays(mpo, mps)
    result = zipup_mpo_mps(CuArray.(host.mpo), CuArray.(host.mps); maxdim)
    return (mps=_zipup_arrays_to_mps(result.mps, host.physical_outputs), log_gauge=result.log_gauge)
end
