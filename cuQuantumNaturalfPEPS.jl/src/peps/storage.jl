...
function _cfg_of(
    ...
    return QnpepsConfig(
        ...
        chi_dl=chi_dl,
        seed=seed,
        dlenv_truncation_route=dlenv_truncation_route,
        sampler_truncation_route=sampler_truncation_route,
        dlenv_density_cutoff=dlenv_density_cutoff,
...
function _validate(arrays::AbstractMatrix)::Nothing
    ...
    for row in 1:lx, col in 1:ly
        A = arrays[row, col]
        if ndims(A) != 5
            throw(DimensionMismatch("site ($row,$col) is rank $(ndims(A)), expected 5 [p,u,r,d,l]"))
        end
        if size(A, AX_P) != dim_phys
            throw(DimensionMismatch("site ($row,$col) physical dim must be uniform ($dim_phys)"))
        end
    end
    for col in 1:ly
...
function random_unitary_peps(
    ...
    2 <= lx <= typemax(Int32) || throw(ArgumentError("lx must fit Int32 and be at least 2"))
    2 <= ly <= typemax(Int32) || throw(ArgumentError("ly must fit Int32 and be at least 2"))
    1 <= dim_bond <= typemax(Int32) ||
        throw(ArgumentError("dim_bond must fit Int32 and be positive"))
    1 <= dim_phys <= typemax(Int32) ||
        throw(ArgumentError("dim_phys must fit Int32 and be positive"))
    0 <= seed <= typemax(UInt64) || throw(ArgumentError("seed must fit UInt64"))
    isfinite(alpha) && alpha >= 0 || throw(ArgumentError("alpha must be finite and nonnegative"))
    config = QnpepsConfig(; lx, ly, dim_bond, chi_s=dim_bond, chi_dl=dim_bond, dim_phys)
    peps_bytes = _peps_bytes(; config)
    peps_bytes > 0 && peps_bytes % sizeof(ComplexF32) == 0 || error("native PEPS size query failed")
    data = CuArray{ComplexF32}(undef, div(peps_bytes, sizeof(ComplexF32)))
    try
        GC.@preserve data begin
            _ffi_random_unitary_peps(; config, peps=pointer(data), peps_bytes, seed, alpha)
        end
    catch
        CUDA.unsafe_free!(data)
        rethrow()
    end
    return CuPeps(data, Int(lx), Int(ly), Int(dim_phys), Int(dim_bond))
end
