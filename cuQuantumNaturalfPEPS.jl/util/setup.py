...
def setup_signature() -> str:
    ...
    fields = (
        ...
        ("cuda-architectures", os.environ.get("QNPEPS_CUDA_ARCHITECTURES", "")),
        ("platform", os.environ.get("QNPEPS_PLATFORM_ID", "")),
...
def setup(force: bool, show_path: bool) -> int:
    if not active_for_this_checkout():
        ...
        return 1
    if os.environ.get("QNPEPS_PLATFORM_ID", "").startswith("jusuf-"):
        print(
            "[setup.py] JUSUF Julia setup awaits live V100 qualification; "
            "use util/build_cuda.sh for the native CUDA 12 build",
            file=sys.stderr,
        )
        return 1
    try:
