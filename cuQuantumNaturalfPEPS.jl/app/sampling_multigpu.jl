...
function sampling_multigpu(arguments=ARGS)::Nothing
    options = parse_app_options("sampling_multigpu.jl", arguments)
    app_dry_run("sampling_multigpu.jl", options) && return nothing
    if !CUDA.functional()
...
abspath(PROGRAM_FILE) == (@__FILE__) && sampling_multigpu()
