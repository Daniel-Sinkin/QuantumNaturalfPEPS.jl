...
function sampling(arguments=ARGS)::Nothing
    options = parse_app_options("sampling.jl", arguments)
    app_dry_run("sampling.jl", options) && return nothing
    if !CUDA.functional()
...
abspath(PROGRAM_FILE) == (@__FILE__) && sampling()
