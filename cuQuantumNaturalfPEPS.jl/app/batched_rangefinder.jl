...
function batched_rangefinder_example(arguments=ARGS)::Nothing
    options = parse_app_options("batched_rangefinder.jl", arguments)
    app_dry_run("batched_rangefinder.jl", options) && return nothing
    if !CUDA.functional()
...
abspath(PROGRAM_FILE) == (@__FILE__) && batched_rangefinder_example()
