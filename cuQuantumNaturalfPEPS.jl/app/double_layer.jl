# Invokes the double layer function.
using cuQuantumNaturalfPEPS
using CUDA

include(joinpath(@__DIR__, "common.jl"))

function double_layer_example(arguments=ARGS)::Nothing
    options = parse_app_options("double_layer.jl", arguments)
    app_dry_run("double_layer.jl", options) && return nothing
    if !CUDA.functional()
...
abspath(PROGRAM_FILE) == (@__FILE__) && double_layer_example()
